    ...
    float3 calculateSpecular(in float4 albedo)
    {
        ...
        if(float(1) == 1) // Realistic
        {
            ...
            if(float(0))
            {
                ...
                finalSpecular += calculateNewSpecular(specularMap.rgb, float(1), realisticAlbedo, float4(1,1,1,1), metallic, float(0.866) * specularMap.a, poiLight.dotNH, poiLight.dotLH, poiLight.color, attenuation);
            #else
                finalSpecular += calculateNewSpecular(specularMap.rgb, float(1), realisticAlbedo, float4(1,1,1,1), metallic, float(0.866) * specularMap.a, poiLight.dotNH, poiLight.dotLH, poiLight.color, poiLight.attenuation);
            #endif
        }
        ...
        if(float(1) == 4)
        {
            ...
            finalSpecular += toonAnisoSpecular(float(0), float(0), poiLight.direction, poiLight.halfDir, specularMap, poiLight.nDotL, float(0.737), float(1.227), float4(1,1,1,1), albedo, metallic, jitter, float(0), packedTangentMap);
            ...
            if (float(1) == 3) // anisotropic
            {
                ...
                finalSpecular += AnisotropicSpecular(float(0), float(0), float(0.866), float(0), float(1), float(1), float4(1,1,1,1), metallic, specularMap, poiLight.color, poiLight.direction, poiLight.halfDir, poiLight.nDotL, jitter, packedTangentMap, albedo);
                finalSpecular *= float4(1,1,1,1);
                finalSpecular *= attenuation;
            }
        #endif
            ...
            for (int index = 0; index < 4; index ++)
            {
                ...
                if (float(1) == 1) // Realistic
                {
                    finalSpecular += calculateNewSpecular(specularMap.rgb, float(1), realisticAlbedo, float4(1,1,1,1), metallic, float(0.866) * specularMap.a, poiLight.vDotNH[index], poiLight.vDotLH[index], poiLight.vColor[index], poiLight.vAttenuationDotNL[index]);
                }
            }
        #endif
        ...
        if (float(1))
        {
            ...
            if(float(1) == 1) // Realistic
            {
                ...
                #ifdef FORWARD_BASE_PASS
                    finalSpecular1 = calculateNewSpecular(specularMap1.rgb, float(0), realisticAlbedo1, float4(1,1,1,1), metallic1, float(0.66) * specularMap1.a, poiLight.dotNH, poiLight.dotLH, poiLight.color, saturate(poiLight.nDotL));
                #else
                    finalSpecular1 = calculateNewSpecular(specularMap1.rgb, float(0), realisticAlbedo1, float4(1,1,1,1), metallic1, float(0.66) * specularMap1.a, poiLight.dotNH, poiLight.dotLH, poiLight.color, poiLight.attenuation);
                #endif
            }
            ...
            if(float(1) == 4)
            {
                ...
                finalSpecular1 += toonAnisoSpecular(float(0), float(0), poiLight.direction, poiLight.halfDir, specularMap1, poiLight.nDotL, float(0.652), float(1.026), float4(1,1,1,1), albedo, metallic1, jitter, float(0), packedTangentMap);
            ...
            if(float(1) == 2) // Toon
            {
                finalSpecular1 = calculateToonSpecular(albedo, poiMesh.uv[0], float4(0,0.05000001,0,1), metallic1, float(0), specularMap1, poiLight.color, poiMesh.normals[float(1)], poiLight.halfDir, poiLight.attenuation);
            ...
            if (float(1) == 3) // anisotropic
            {
                ...
                finalSpecular1 = AnisotropicSpecular(float(0), float(0), float(0.66), float(0), float(1), float(1), float4(1,1,1,1), metallic1, specularMap1, poiLight.color, poiLight.direction, poiLight.halfDir, poiLight.nDotL, jitter, packedTangentMap, albedo);
                finalSpecular1 *= float4(1,1,1,1);
                finalSpecular1 *= poiLight.attenuation;
            }
                ...
                // Non Important Lights
                    ...
                    for (int index = 0; index < 4; index ++)
                    {
                        ...
                        if (float(1) == 1) // Realistic
                        {
                            finalSpecular1 += calculateNewSpecular(specularMap1.rgb, float(0), realisticAlbedo1, float4(1,1,1,1), metallic1, float(0.66) * specularMap1.a, poiLight.vDotNH[index], poiLight.vDotLH[index], poiLight.vColor[index], poiLight.vAttenuationDotNL[index]);
                        }
                    }
                #endif
            #endif
