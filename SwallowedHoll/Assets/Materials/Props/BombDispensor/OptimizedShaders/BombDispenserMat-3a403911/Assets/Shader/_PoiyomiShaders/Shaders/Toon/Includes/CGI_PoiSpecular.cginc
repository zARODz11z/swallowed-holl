    ...
    float3 calculateSpecular(in float4 albedo)
    {
        ...
        if(float(4) == 1) // Realistic
        {
            ...
            if(float(1))
            {
                ...
                finalSpecular += calculateNewSpecular(specularMap.rgb, float(0), realisticAlbedo, float4(1,1,1,1), metallic, float(0.626) * specularMap.a, poiLight.dotNH, poiLight.dotLH, poiLight.color, attenuation);
            #else
                finalSpecular += calculateNewSpecular(specularMap.rgb, float(0), realisticAlbedo, float4(1,1,1,1), metallic, float(0.626) * specularMap.a, poiLight.dotNH, poiLight.dotLH, poiLight.color, poiLight.attenuation);
            #endif
        }
        ...
        if(float(4) == 4)
        {
            ...
            finalSpecular += toonAnisoSpecular(float(0), float(0), poiLight.direction, poiLight.halfDir, specularMap, poiLight.nDotL, float(0.755), float(1.398), float4(1,1,1,1), albedo, metallic, jitter, float(0), packedTangentMap);
            ...
            if (float(4) == 3) // anisotropic
            {
                ...
                finalSpecular += AnisotropicSpecular(float(0), float(0), float(0.626), float(0), float(1), float(1), float4(1,1,1,1), metallic, specularMap, poiLight.color, poiLight.direction, poiLight.halfDir, poiLight.nDotL, jitter, packedTangentMap, albedo);
                finalSpecular *= float4(1,1,1,1);
                finalSpecular *= attenuation;
            }
        #endif
            ...
            for (int index = 0; index < 4; index ++)
            {
                ...
                if (float(4) == 1) // Realistic
                {
                    finalSpecular += calculateNewSpecular(specularMap.rgb, float(0), realisticAlbedo, float4(1,1,1,1), metallic, float(0.626) * specularMap.a, poiLight.vDotNH[index], poiLight.vDotLH[index], poiLight.vColor[index], poiLight.vAttenuationDotNL[index]);
                }
            }
        #endif
