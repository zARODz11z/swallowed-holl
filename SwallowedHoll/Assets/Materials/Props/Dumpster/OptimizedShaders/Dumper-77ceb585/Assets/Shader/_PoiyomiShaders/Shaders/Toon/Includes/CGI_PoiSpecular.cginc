    ...
    float3 calculateSpecular(in float4 albedo)
    {
        ...
        half metallic = POI2D_SAMPLER_PAN(_SpecularMetallicMap, _MainTex, poiMesh.uv[float(0)], float4(0,0,0,0)).r * float(0.357);
        ...
        if(float(4) == 1) // Realistic
        {
            ...
            if(float(0))
            {
                ...
                finalSpecular += calculateNewSpecular(specularMap.rgb, float(0), realisticAlbedo, float4(1,1,1,1), metallic, float(0.922) * specularMap.a, poiLight.dotNH, poiLight.dotLH, poiLight.color, attenuation);
            #else
                finalSpecular += calculateNewSpecular(specularMap.rgb, float(0), realisticAlbedo, float4(1,1,1,1), metallic, float(0.922) * specularMap.a, poiLight.dotNH, poiLight.dotLH, poiLight.color, poiLight.attenuation);
            #endif
        }
        ...
        if(float(4) == 4)
        {
            ...
            finalSpecular += toonAnisoSpecular(float(0), float(0), poiLight.direction, poiLight.halfDir, specularMap, poiLight.nDotL, float(0.749), float(1.336), float4(1,1,1,1), albedo, metallic, jitter, float(0), packedTangentMap);
            ...
            if(float(4) == 2) // Toon
            {
                finalSpecular += calculateToonSpecular(albedo, poiMesh.uv[0], float4(0.1241885,0.4031057,0,1), metallic, float(0), specularMap, poiLight.color, poiMesh.normals[float(1)], poiLight.halfDir, poiLight.attenuation);
            ...
            if (float(4) == 3) // anisotropic
            {
                ...
                finalSpecular += AnisotropicSpecular(float(0), float(0), float(0.922), float(0), float(1), float(1), float4(1,1,1,1), metallic, specularMap, poiLight.color, poiLight.direction, poiLight.halfDir, poiLight.nDotL, jitter, packedTangentMap, albedo);
                finalSpecular *= float4(1,1,1,1);
                finalSpecular *= attenuation;
            }
        #endif
            ...
            for (int index = 0; index < 4; index ++)
            {
                ...
                if (float(4) == 1) // Realistic
                {
                    finalSpecular += calculateNewSpecular(specularMap.rgb, float(0), realisticAlbedo, float4(1,1,1,1), metallic, float(0.922) * specularMap.a, poiLight.vDotNH[index], poiLight.vDotLH[index], poiLight.vColor[index], poiLight.vAttenuationDotNL[index]);
                }
            }
        #endif
