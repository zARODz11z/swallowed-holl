    ...
    float3 calculateSpecular(in float4 albedo)
    {
        ...
        if(float(4) == 4)
        {
            ...
            finalSpecular += toonAnisoSpecular(float(0), float(0), poiLight.direction, poiLight.halfDir, specularMap, poiLight.nDotL, float(0.97), float(1.024), float4(1,1,1,1), albedo, metallic, jitter, float(0), packedTangentMap);
            ...
            if(float(4) == 2) // Toon
            {
                finalSpecular += calculateToonSpecular(albedo, poiMesh.uv[0], float4(0.25,0.3,0,1), metallic, float(0), specularMap, poiLight.color, poiMesh.normals[float(0)], poiLight.halfDir, poiLight.attenuation);
