    ...
    float3 calculateSpecular(in float4 albedo)
    {
        ...
        if(float(4) == 4)
        {
            ...
            finalSpecular += toonAnisoSpecular(float(0), float(0), poiLight.direction, poiLight.halfDir, specularMap, poiLight.nDotL, float(0.721), float(1.724), float4(1,1,1,1), albedo, metallic, jitter, float(0), packedTangentMap);
