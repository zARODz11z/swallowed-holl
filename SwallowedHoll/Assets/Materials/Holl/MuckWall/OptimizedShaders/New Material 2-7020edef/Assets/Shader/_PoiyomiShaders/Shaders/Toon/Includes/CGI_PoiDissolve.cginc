    ...
    void calculateDissolve(inout float4 albedo, inout float3 dissolveEmission)
    {
        ...
        if(float(1) == 1) // Basic
        {
            ...
            //noise = remap(noise, 0, 1, float(0.021), 1 - float(0.021));
            dissolveAlpha = dissolveAlpha >= noise;
            edgeAlpha = remapClamped(noise, da + float(0.021), da, 0, 1) * (1 - dissolveAlpha);
        }
