    ...
    void applyGlitter(inout float4 albedo, inout float3 glitterEmission)
    {
        ...
        // Scale
        float2 st = poiMesh.uv[0] * float(307.39);
        ...
        switch(float(0))
        {
            ...
            finalGlitter = lerp(0, float(0) * glitterAlpha, glitterAlpha) + max(pow(saturate(dot(lerp(glitterReflectionDirection, poiCam.viewDir, float(0.881)), poiCam.viewDir)), float(308)), 0);
        ...
        half3 glitterColor = float4(0,0.09776545,1,1);
        ...
        if(float(0) == 1)
        {
            albedo.rgb = lerp(albedo.rgb, finalGlitter * glitterColor * float(9.9), finalGlitter * glitterTexture.a * glitterMask);
            glitterEmission = finalGlitter * glitterColor * max(0, (float(9.9) - 1) * glitterTexture.a) * glitterMask;
        }
        else
        {
            glitterEmission = finalGlitter * glitterColor * float(9.9) * glitterTexture.a * glitterMask;
        }
    }
