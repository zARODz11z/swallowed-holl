    ...
    float3 calculateEmissionNew(in float4 baseColor, inout float4 finalColor)
    {
        ...
        if (!float(0))
        {
            emissionColor0 = POI2D_SAMPLER_PAN(_EmissionMap, _MainTex, poiMesh.uv[float(0)], float4(0,0,0,0)) * lerp(1, baseColor, float(0)).rgb * float4(0,0.266294,1,1).rgb;
        }
        else
        {
            emissionColor0 = UNITY_SAMPLE_TEX2D_SAMPLER(_EmissionMap, _MainTex, ((.5 + poiLight.nDotV * .5) * float4(0.04,0.04,0,0).xy) + _Time.x * float(5)) * lerp(1, baseColor, float(0)).rgb * float4(0,0.266294,1,1).rgb;
        }
