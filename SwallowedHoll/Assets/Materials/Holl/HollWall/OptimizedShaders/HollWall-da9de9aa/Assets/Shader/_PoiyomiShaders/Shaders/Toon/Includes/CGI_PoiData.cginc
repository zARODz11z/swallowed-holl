    ...
    void calculateVertexLightingData(in v2f i)
    {
            ...
            for (int index = 0; index < 4; index ++)
            {
                ...
                if (float(0) == 1)
                {
                    ...
                    poiLight.vColor[index] = min(poiLight.vColor[index], poiLight.vColor[index] / (intensity / float(0.96)));
                }
    ...
    void calculateLightingData(in v2f i)
    {
            ...
            poiLight.color = magic + normalLight;
                ...
                if (float(0) == 1)
                {
                    ...
                    poiLight.color = min(poiLight.color, poiLight.color / (additiveLightIntensity / float(0.96)));
                }
            #endif
        #endif
    ...
    {
        half3 mainNormal = UnpackScaleNormal(POI2D_SAMPLER_PAN(_BumpMap, _MainTex, poiMesh.uv[float(0)], float4(0,0,0,0)), float(0.17));
        #ifdef FINALPASS
