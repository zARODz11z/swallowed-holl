    ...
    float3 calculateSpecular(in float4 albedo)
    {
        ...
        if(float(1) == 1) // Realistic
        {
            ...
            if(float(0))
            {
                ...
                finalSpecular += calculateNewSpecular(specularMap.rgb, float(0), realisticAlbedo, float4(1,1,1,1), metallic, float(0.532) * specularMap.a, poiLight.dotNH, poiLight.dotLH, poiLight.color, attenuation);
            #else
                finalSpecular += calculateNewSpecular(specularMap.rgb, float(0), realisticAlbedo, float4(1,1,1,1), metallic, float(0.532) * specularMap.a, poiLight.dotNH, poiLight.dotLH, poiLight.color, poiLight.attenuation);
            #endif
        }
        ...
        if(float(1) == 4)
        {
            ...
            if (float(1) == 3) // anisotropic
            {
                ...
                finalSpecular += AnisotropicSpecular(float(0), float(0), float(0.532), float(0), float(1), float(1), float4(1,1,1,1), metallic, specularMap, poiLight.color, poiLight.direction, poiLight.halfDir, poiLight.nDotL, jitter, packedTangentMap, albedo);
                finalSpecular *= float4(1,1,1,1);
                finalSpecular *= attenuation;
            }
        #endif
            ...
            for (int index = 0; index < 4; index ++)
            {
                ...
                if (float(1) == 1) // Realistic
                {
                    finalSpecular += calculateNewSpecular(specularMap.rgb, float(0), realisticAlbedo, float4(1,1,1,1), metallic, float(0.532) * specularMap.a, poiLight.vDotNH[index], poiLight.vDotLH[index], poiLight.vColor[index], poiLight.vAttenuationDotNL[index]);
                }
            }
        #endif
